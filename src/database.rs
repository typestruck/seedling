...
pub fn connect() -> Client {
    return Client::connect("host=localhost user=melanchat db=melanchat", NoTls).expect("Couldn't connect to database!");
}

pub fn create_grades(client: &mut Client) -> Vec<Grade> {
    ...
    let query = "
        ...
        from    (with k as  (select id,
                                    ...
                                    0 achievements,
                                    0 backer_bonus,
                                    date_part('week', age(now() at time zone 'utc', joined)) <= 1 is_new_user
                            from users u
                            order by karma)
                select id,
                       is_new_user,
    ...
    for row in client.query(query, &[]).expect("Couldn't query!") {
        grades.push(Grade {
            id: row.get(0),
            score: row.get(1),
...
pub fn create_suggestion_lists(grades: &[Grade]) -> String {
    ...
    for (i, gr) in grades.iter().enumerate() {
        let position = if gr.is_new_user { 3 } else { (i as f32 / bins as f32).ceil() as u32};
        ...
        query.push_str(&format!("({},{},{},{},{},{}){}", gr.id, list_a, list_b, list_c, list_d, list_e, token));
    }
